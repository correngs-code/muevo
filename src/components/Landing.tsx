  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
        <div style={{ flex: 1, background: 'rgba(59,108,255,0.2)', border: '1px solid rgba(59,108,255,0.3)', borderRadius: 14, padding: '10px', textAlign: 'center', fontSize: 13, color: '#7aa2ff', fontWeight: 600 }}>+ Entrata</div>
        <div style={{ flex: 1, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 14, padding: '10px', textAlign: 'center', fontSize: 13, color: 'rgba(255,255,255,0.7)', fontWeight: 600 }}>− Spesa</div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 8 }}>
        {categories.map((emoji, i) => (
          <div key={i} style={{ width: 36, height: 36, background: 'rgba(255,255,255,0.06)', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>{emoji}</div>
        ))}
      </div>
      <div style={{ marginTop: 16 }}>
        {[
          ...
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: i < 2 ? '1px solid rgba(255,255,255,0.06)' : 'none' }}>
            <div style={{ width: 32, height: 32, background: 'rgba(255,255,255,0.08)', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>{tx.icon}</div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, color: '#fff', fontWeight: 500 }}>{tx.name}</div>
              <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)' }}>{tx.time}</div>
            </div>
            <div style={{ fontSize: 13, color: tx.amount.startsWith('+') ? '#4ade80' : 'rgba(255,255,255,0.7)', fontWeight: 600 }}>{tx.amount}</div>
          </div>
        ))}
      </div>
    </div>
...
export default function Landing({ onSignIn, onContinueLocal }: LandingProps) {
  return (
    <div className="landing-root" style={{
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <MuevoLogo size={26} />
          <span style={{ fontSize: 20, fontWeight: 800, letterSpacing: '-0.03em', color: '#fff' }}>Muevo</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <GhostButton onClick={onContinueLocal} style={{ color: 'rgba(255,255,255,0.7)', opacity: 1 }}>
            Senza account
          </GhostButton>
          {hasEnv && (
            <CtaButton onClick={onSignIn} size="sm">
              Accedi con Google
            </CtaButton>
          )}
        </div>
      ...
      <section style={{
        ...
        <div>
          ...
          <p style={{ margin: '0 0 36px', fontSize: 18, lineHeight: 1.65, color: 'rgba(255,255,255,0.62)', maxWidth: 460 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, alignItems: 'flex-start', marginBottom: 36 }}>
            {hasEnv ? (
              <>
                <CtaButton onClick={onSignIn} size="lg">
                  Accedi con Google
              ...
              <>
                <CtaButton onClick={onContinueLocal} size="lg">
                  Inizia (dati salvati sul dispositivo)
                  <IconArrowRight size={18} />
                </CtaButton>
                <div style={{
                  background: 'rgba(255,200,0,0.08)', border: '1px solid rgba(255,200,0,0.2)',
                  borderRadius: 12, padding: '10px 16px', fontSize: 12,
                  color: 'rgba(255,200,100,0.85)', maxWidth: 460,
                }}>
                  Login Google non configurato. I dati saranno salvati solo localmente nel browser.
                </div>
              </>
            )}
          </div>
      ...
      <section style={{ maxWidth: 900, margin: '0 auto', padding: '80px 24px', textAlign: 'center' }}>
        ...
        }}>Semplice come 1, 2, 3</h2>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 20 }}>
          {[
            { step: '01', emoji: '🔐', title: 'Accedi con Google', desc: 'Un click e sei dentro. Nessuna registrazione noiosa, nessuna password da ricordare.' },
            { step: '02', emoji: '⚡', title: 'Aggiungi spese', desc: 'Scrivi "caffè 1,50" o usa il microfono. L\'icona e la categoria vengono riconosciute in automatico.' },
            { step: '03', emoji: '📊', title: 'Vedi dove vanno', desc: 'Il saldo aggiornato, le categorie, le tendenze mensili — tutto chiaro, tutto al volo.' },
          ].map((item) => (
